...
export class StartggApi extends LitElement {
            ...
            replicant.once('change', () => {
                ...
                if (numDeclared >= replicants.length) {
                    startgg.on('change', (newVal) => {
                        ...
                        //Top 8 Generation
                        if (
                            !('selectedEvent' in newVal) ||
                            newVal.selectedEvent < 0
                        )
                            this.topSelectedEventIndex = '';
                        else
                            this.topSelectedEventIndex =
                                newVal.selectedEvent.toString();
    ...
    updated(changedProperties) {
        //Stream Queues
        this.renderRoot.querySelector('#selectedQueue').renderer = function (
            root
        ) {
        ...
        //Events
        this.renderRoot.querySelector('#eventSelector').renderer = function (
            root
        ) {
            //Check if there is a list-box generated with the previous renderer call to update its content instead of recreation
            if (root.firstChild) {
    ...
    _tournamentSlugChange(event) {
        ...
        if (
            newEventURL.includes('start.gg/tournament/') ||
            newEventURL.includes('smash.gg/tournament/')
        ) {
    ...
    _boThresholdValueChange(event) {
        ...
        if (newValue < -1 || newValue > 255) return;
    ...
    _topGenerateButtonClicked() {
        ...
        nodecg.sendMessage(
            'api_startgg_generateTopStandings',
            0,
            (error, result) => {
                this.topStandingsGenerating = false;
