    ...
    if (startgg.value.selectedQueue >= 0) {
        ...
        if (activeQueue) {
            if (tournyData.data.tournament.streamQueue) {
                matchingQueue = tournyData.data.tournament.streamQueue.find(
                    (elem) => {
    ...
    if (startgg.value.selectedEvent >= 0) {
        ...
        if (activeEvent) {
            if (tournyData.data.tournament.events) {
                matchingEvent = tournyData.data.tournament.events.find(
                    (elem) => {
...
async function generateTopStandings(ack = null) {
    ...
    try {
        ...
        for (let standing of standings) {
            ...
            if (standing.entrant) {
                ...
                let latestSetRes = await pullLatestMatchDataFromEntrant(
                    standing.entrant.id,
                    1,
                    1
                )
